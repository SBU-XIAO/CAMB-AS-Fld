    ...
    !KZ
    this%astress_model = Ini%Read_Double('astress_model', 1.d0) 
    ...
    subroutine TDarkEnergyFluid_PerturbedStressEnergy(this, dgrhoe, dgqe, dgpie, & !KZ
    ...
    subroutine TDarkEnergyFluid_PerturbationEvolve(this, ayprime, w, w_ix, &
        a, adotoa, k, z, y, dgrho, dgrho_de, dgpi_de, grho, grhov_t) !KZ: take dgrho of other component as input
    class(TDarkEnergyFluid), intent(in) :: this
    real(dl), intent(inout) :: ayprime(:)
    real(dl), intent(in) :: a, adotoa, w, k, z, y(:), dgrho, dgrho_de, dgpi_de, grho, grhov_t!KZ
    integer, intent(in) :: w_ix
    ...
    if (abs(w+1) > 1e-6) then
        ...
        !KZ: evolution eq with Astress:
        ayprime(w_ix + 1) = ayprime(w_ix + 1) -2.0/3.0 *k / (1 + w) * dgpi_de  / grhov_t
    else
        ayprime(w_ix + 1) = 0
    end if
    ...
    subroutine TAxionEffectiveFluid_PerturbationEvolve(this, ayprime, w, w_ix, &
        a, adotoa, k, z, y, dgrho, dgrho_de, dgpi_de, grho, grhov_t) !KZ
    ...
    subroutine TAxionEffectiveFluid_PerturbedStressEnergy(this, dgrhoe, dgqe, dgpie, & !KZ: just for compiling
        a, dgq, dgrho, grho, grhov_t, w, gpres_noDE, etak, adotoa, k, kf1, ay, ayprime, w_ix)
    class(TAxionEffectiveFluid), intent(inout) :: this
    real(dl), intent(out) :: dgrhoe, dgqe, dgpie !KZ: just for compiling
    real(dl), intent(in) :: a, dgq, dgrho, grho, grhov_t, w, gpres_noDE, etak, adotoa, k, kf1
    real(dl), intent(in) :: ay(*)
    real(dl), intent(inout) :: ayprime(*)
    integer, intent(in) :: w_ix
